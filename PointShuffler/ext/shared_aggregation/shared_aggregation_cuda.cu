                                        ...
                                        cudaStream_t stream)
{
    ...
    if(block.x > 1024)
        block.x =1024;
    shared_aggregation_kernel<<<grid, block, 0, stream>>>(n, block_num, channel, shared_k, centers, have_center, mlp_result, searching_offset, _1center_in_group, ns_index, shared_len, result);
}
