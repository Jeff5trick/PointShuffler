...
def uniform_sampling(xyz,npoint,step,block_size, xyz_offset):
    ...
    return fps_idx, xyz_centers, u_order, u_len, u_offset, point2group
...
def neighbor_search_common (block_size, hop, nsample, radius,centers, point2group, u_len, u_offset, u_order, xyz):
    ...
    return have_center, isn_shared, searching_length, searching_offset, _1center_in_group, ns_index, shared_len, neighbor_len
...
class PointShufflerSetAbstraction(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        if self.group_all:
            ...
            fps_idx, xyz_centers, u_order, u_len, u_offset, point2group = uniform_sampling(xyz, self.npoint, self.step, self.block_size, self.xyz_offset)

            have_center, isn_shared, searching_length, searching_offset,_1center_in_group, ns_index, shared_len, neighbor_len = neighbor_search_common(self.block_size, self.hop, self.nsample, self.radius, fps_idx,point2group,u_len,u_offset,u_order,xyz)          
